class LangfuseClient {
  constructor(config) {
    ...
    this.sdkVersion = config.sdkVersion || 'unknown';
  ...
  async ingest(events) {
    ...
    try {
      ...
      if (response.status === 408 || response.status === 429 || response.status >= 500) {
        ...
        return {
          ok: false,
          error: response.status === 429 ? 'rate_limit' : 'server',
      ...
      if (response.status >= 400 && response.status < 500) {
        const errorText = await response.text().catch(() => 'Unknown error');
        return { ok: false, error: 'client', status: response.status, details: errorText };
      }
